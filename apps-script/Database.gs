    ...
    for (var i = 1; i < data.length; i++) {
      if (data[i][SESSIONS_COLUMNS.SESSION_ID] === sessionId) {
        ...
        return { success: true, data: {
          SESSION_ID: data[i][SESSIONS_COLUMNS.SESSION_ID],
          RUN_ID:     runId,
          DAY_NUM:    data[i][SESSIONS_COLUMNS.DAY_NUM],
          START_TIME: _serializeDate(data[i][SESSIONS_COLUMNS.START_TIME]),
          END_TIME:   _serializeDate(now),
          STATUS:     'Completed'
        }};
      }
    }
