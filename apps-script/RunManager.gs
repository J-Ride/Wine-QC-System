...
function endProduction(runId) {
  try {
    ...
    try {
      var appendResult = appendProductionSession(runId, closeResult.data);
      if (!appendResult.success) {
        Logger.log('endProduction: appendProductionSession failed for ' + sessionId + ': ' + appendResult.error);
      }
    } catch (e) {
      Logger.log('endProduction: appendProductionSession threw for ' + sessionId + ': ' + e.message);
    }
